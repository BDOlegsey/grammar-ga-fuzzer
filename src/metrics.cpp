// metrics.cpp — corpus metrics and coverage helpers
#include "metrics.h"
#include <algorithm>
#include <cmath>
...
std::size_t absorb(CorpusStats& stats, const ExecutionResult& result,
    ...
    const std::size_t n = stats.num_branches > 0 ? stats.num_branches : result.branch_count;
    std::size_t newly_found = 0;
    for (std::size_t i = 0; i < n && i < kMaxBranches; ++i) {
...
double coverage_ratio(const CorpusStats& stats) {
    if (stats.num_branches == 0) return 0.0;
    std::size_t covered = 0;
    for (std::size_t i = 0; i < stats.num_branches; ++i) {
        if (stats.covered[i]) covered++;
    }
    return static_cast<double>(covered) / static_cast<double>(stats.num_branches);
}
...
double diversity_score(const std::array<std::size_t, kMaxBranches>& branch_hit_counts,
                       std::size_t num_branches) {
    std::size_t total = 0;
    for (std::size_t i = 0; i < num_branches; ++i) {
        total += branch_hit_counts[i];
    }
    if (total == 0 || num_branches == 0) return 0.0;

    double entropy = 0.0;
    for (std::size_t i = 0; i < num_branches; ++i) {
        const auto c = branch_hit_counts[i];
        if (c == 0) continue;
        double p = static_cast<double>(c) / static_cast<double>(total);
        entropy -= p * std::log2(p);
    }
    double max_ent = std::log2(static_cast<double>(num_branches));
