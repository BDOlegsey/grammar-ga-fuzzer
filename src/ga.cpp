// ga.cpp — genetic algorithm: baseline and adaptive experiments
#include "ga.h"
#include "generator.h"
#include "metrics.h"
#include "utils.h"
...
Candidate evaluate_candidate(
    ...
    std::array<bool, kMaxBranches>& globally_covered,
    std::array<std::size_t, kMaxBranches>& global_hit_counts
) {
    const std::size_t n = sut.branch_count();
    CorpusStats local_stats;
    local_stats.num_branches = n;

    std::array<std::size_t, kMaxBranches> local_hit_counts{};
    ...
    double bugs = normalized_bug_score(local_stats, 10);
    double nov = static_cast<double>(local_stats.novelty_hits) / static_cast<double>(n);
    double div = diversity_score(local_hit_counts, n);
...
double population_diversity(const std::vector<Candidate>& pop, std::size_t num_branches) {
    if (pop.size() < 2) return 0.0;
    std::array<std::size_t, kMaxBranches> total_counts{};
    for (const auto& c : pop) {
        for (std::size_t i = 0; i < num_branches; ++i) {
            total_counts[i] += c.branch_hits[i];
        }
    }
    return diversity_score(total_counts, num_branches);
}

double average_pop_input_length(const std::vector<Candidate>& pop) {
    if (pop.empty()) return 0.0;
    double sum = 0.0;
    for (const auto& c : pop) {
        sum += avg_input_length(c.stats);
    }
    return sum / static_cast<double>(pop.size());
}
...
Individual crossover(const Individual& p1, const Individual& p2, std::mt19937& rng, double crossover_rate) {
    ...
    for (const auto& [nt, vec1] : p1) {
        ...
        for (std::size_t i = 0; i < vec1.size(); ++i) {
            constexpr double alpha_blx = 0.3;
...
AdaptiveResult run_adaptive_experiment(const Grammar& grammar, const GAConfig& cfg,
    ...
    for (int gen = 0; gen < cfg.generations; ++gen) {
        for (auto& cand : population) {
            cand = evaluate_candidate(grammar, cfg, sut, cand.ind, rng, overall,
        ...
        for (std::size_t i = 0; i < n; ++i) {
            if (globally_covered[i]) prev_covered++;
        }
        ...
        rec.cumulative_bugs = overall.unique_crashes.size();
        rec.best_bugs = rec.cumulative_bugs;
        rec.novelty = static_cast<double>(prev_covered) / static_cast<double>(n);
        rec.diversity = population_diversity(population, n);
        rec.avg_input_length = avg_input_length(population.front().stats);
        rec.avg_input_length_pop = average_pop_input_length(population);

        const bool record_weights = (gen % 5 == 0) || (gen == cfg.generations - 1);
        if (record_weights) {
            rec.has_weights = true;
            rec.best_rule_weights = std::map<std::string, std::vector<double>>(
                population.front().ind.begin(), population.front().ind.end());
        }
        ...
        if (gen > cfg.generations / 2) {
            adaptive_mut = cfg.mutation_rate * 0.7;
        }
        if (gen > 3 * cfg.generations / 4) {
            adaptive_mut = cfg.mutation_rate * 0.4;
        }
    ...
    summary.mutation_rate = cfg.mutation_rate;
    summary.crossover_rate = cfg.crossover_rate;
    summary.novelty_score = static_cast<double>(std::count(globally_covered.begin(),
                                                           globally_covered.begin() + n, true))
                          / static_cast<double>(n);
    summary.diversity_score = population_diversity(population, n);
    summary.avg_input_length = avg_input_length(overall);
